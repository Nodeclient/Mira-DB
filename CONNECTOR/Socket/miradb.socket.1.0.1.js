var MiraDB = function(user,password,server,database) {	
    ...
    client.Connect = function() {
        if(client.Jconn==false){ 
            ...
            client.socket.on('connect', function(data) { 
                ...
                client.Event({"resume":true,"code":"JCL_0","text": `Connection Success` });
            });	
            ...
            client.socket.on('EventData', function (data)  {
                switch (data[1]) {
                    ...
                    case "LOGIN_SUCCESS":
                        ...
                        break;
                    case "LOGIN_ERROR":
                        client.Event({"resume":true,"code":"JCL_0","text": `Connection Failed` });
                        break;
                    default:
                        break;
                }
            ...
            client.socket.on('disconnect', function(data) {      
                client.Jconn=false; 
                client.socket.disconnect(); 
                client.Event({"resume":false,"code":"JCL_1001","text": `Disconnect JSQL Connection` }); 
            });	
            
            client.socket.on('connect_error', function(data)  {
                client.Jconn=false; 
                client.socket.disconnect(); 
                client.Event({"resume":false,"code":"JCL_1002","text": `(${client.server}) Server is currently offline` }); 
            });	

        } else {
                client.Event({"resume":false,"code":"JCL_4001","text": `Connection already open` });
        }
    };

    client.Event = function(e,callback) {
    ...
    client.Query = function(str,callback) {
        if(client.Jconn==true){				
            ...
            client.Event({"resume":false,"code":"JCL_2007","text": `No server connection` });				 
        } 		
    };
